...
resource "aws_eks_access_entry" "main" {
    ...
    principal_arn = var.principal_arn # means the Amazon Resource Name (ARN) of the IAM principal (user, role, or service) that is being granted access to the EKS cluster.
...
resource "aws_eks_access_policy_association" "main" {
    ...
    policy_arn = var.policy_arn # means what permissions are being granted to the IAM principal for accessing the EKS cluster.
    ...
    #means to ensure that the access entry is created before associating the policy with it.
    depends_on = [
